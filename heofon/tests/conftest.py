...
def pytest_runtest_setup(item):
    ...
    folder_path = utils_file.create_test_output_subfolder(testrun_path, short_name)
...
def initialize_logging():
    ...
    heofon_folder, testrun_folder = utils_file.create_test_output_folder(timestamp)

    # save the heofon folder as a global config
    namespace_data['heofon_folder'] = str(heofon_folder)
...
def set_up_testcase_reporting(path_to_subfolder, fixturenames):
    ...
    if '--collect-only' in sys.argv:
        ...
        requests_folder = str(utils_file.create_test_output_subfolder(output_path, folder_type))
        ...
        folder_type = 'cookies'
        if is_webapp:
            cookie_folder = str(utils_file.create_test_output_subfolder(output_path, folder_type))
        ...
        if is_webapp:
            screenshots_folder = \
                str(utils_file.create_test_output_subfolder(output_path, folder_type))
        ...
        if is_chrome and is_webapp:
            ...
            folder_type = 'network'
            network_folder = str(utils_file.create_test_output_subfolder(output_path, folder_type))
            ...
            folder_type = 'console'
            console_folder = str(utils_file.create_test_output_subfolder(output_path, folder_type))
